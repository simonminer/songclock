...
export default function AnalogClock({ hours, minutes, seconds }: AnalogClockProps) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      ctx.fillText(hourNumber.toString(), textX, textY)
    }
    ...
    drawHand(ctx, centerX, centerY, radius * 0.5, hourAngle, 6, "rgba(255, 130, 130, 1)")

    // Draw minute hand
    const minuteAngle = (minutes * Math.PI) / 30 + (seconds * Math.PI) / (30 * 60)
    drawHand(ctx, centerX, centerY, radius * 0.7, minuteAngle, 4, "rgba(130, 255, 130, 1)")

    // Draw second hand
    const secondAngle = (seconds * Math.PI) / 30
    drawHand(ctx, centerX, centerY, radius * 0.8, secondAngle, 2, "rgba(130, 180, 255, 1)")
